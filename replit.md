...
- **AI Agents Platform**: 6 specialized AI agents powered by OpenAI via Replit AI Integrations (`AI_INTEGRATIONS_OPENAI_BASE_URL` + `AI_INTEGRATIONS_OPENAI_API_KEY`). Shared LLM client at `server/agents/llm.ts` using `openai/gpt-4o-mini` model. DB tables: `moderation_logs`, `agent_interactions`. Columns: `moderationStatus` on listings and posts.
  - **Moderation Agent** (`server/agents/moderationAgent.ts`): Auto-moderates new listings (async, non-blocking on creation). Flags prohibited items, scam indicators, misleading content. Logs all decisions to `moderation_logs`.
  - **Support Agent** (`server/agents/supportAgent.ts`): BarterBot chat assistant. Floating chat widget at `client/src/components/ai-support-chat.tsx` (only for authenticated users). API: `POST /api/ai/support`.
  - **Matching Agent** (`server/agents/matchingAgent.ts`): Smart barter matching based on user offers/needs and available listings. API: `GET /api/ai/matches`.
  - **Valuation Agent** (`server/agents/valuationAgent.ts`): AI pricing advisor for barter items. Panel at `client/src/components/ai-valuation-panel.tsx` integrated in create-listing form. API: `POST /api/ai/valuation`.
  ...
  - **AI Logs Admin Tab**: Admin dashboard "AI Logs" section showing moderation logs and agent interactions. API: `GET /api/ai/logs`.
