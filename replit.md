...
- **Session Management**: Express-session backed by Postgres via `connect-pg-simple` (auto-creates `session` table; reuses pool from `server/db.ts`). `SESSION_SECRET` env var is REQUIRED in every environment — server refuses to boot without it.
...
- **Trade License Gate**: Business accounts require KYB status "APPROVED" to create listings or accept deals; license upload in Settings (`/api/upload?type=business_license`) sets `kybStatus` to "PENDING_REVIEW". Private docs (`verification`, `business_license`) are stored in the private object-storage bucket under `<PRIVATE_OBJECT_DIR>/private-docs/<userId>/<random>.<ext>` with an `owner=userId / visibility=private` ACL; downloads go through `GET /api/private-docs/:userId/:filename` (auth-gated, owner-or-admin). Public uploads (avatar/portfolio) write to `./uploads` with `crypto.randomBytes(24)` filenames. All uploads are magic-byte verified via `file-type` against an allow-list of jpeg/png/gif/webp/pdf — client-supplied MIME and extension are ignored. Admin KYB endpoint (`PATCH /api/admin/users/:id/kyb`) validates with a strict Zod enum (`NOT_STARTED | IN_PROGRESS | PENDING_REVIEW | APPROVED | DECLINED`). `registerSchema`, `loginSchema`, `updateProfileSchema`, `onboardingSchema` use `.strict()` so unknown fields (e.g. attempts to smuggle `isAdmin`/`role`/`kybStatus`) get a 400.
- **Password Reset Flow**: `/forgot-password` and `/reset-password` pages; `POST /api/auth/forgot-password` sends a 1-hour expiring token via email; `GET /api/auth/reset-password/validate` validates token on page load; `POST /api/auth/reset-password` updates password; `server/emailService.ts` uses nodemailer (graceful console fallback if `SMTP_HOST`/`SMTP_USER`/`SMTP_PASS` not set); "Forgot password?" link on login page. Reset tokens are stored as SHA-256 hashes (`hashResetToken` in `server/routes.ts`) — only the raw token appears in the email link. Successful reset destroys ALL of the user's pg sessions; `POST /api/users/change-password` destroys all other sessions but keeps the caller's current one (`destroyUserSessions` helper deletes from the `session` table where `sess->>'userId'` matches).
- **Waitlist Mode**: Toggle with `WAITLIST_MODE=true` env. Public browsing stays open; gated CTAs (Sign In/Register/Login/Onboarding/mobile login tab) redirect to `/` and open `WaitlistDialog` (`client/src/components/waitlist-dialog.tsx`). Provider at `client/src/lib/waitlist.tsx` polls `GET /api/waitlist/mode`, captures `?ref=` to localStorage, exposes `useWaitlist()` and `useAuthGate()`. `waitlist_entries` table (serial PK) with email/name/country/city/accountType/businessName/categoriesOfInterest/source, `referralCode` (8-char base64url uppercase via crypto.randomBytes), `referredByCode`, `referralCount` (incremented on referred signup), `position` (FIFO counter), `confirmedAt`, `convertedUserId`. Routes (`server/waitlistRoutes.ts`): public `GET /api/waitlist/mode|count`, `GET /api/waitlist/by-code/:code`, `POST /api/waitlist` (Zod-validated, honeypot `company_website`, in-memory IP rate-limit 5/hr, fire-and-forget Resend welcome email via `sendWaitlistWelcomeEmail`); admin `GET /api/admin/waitlist` (entries+stats), `GET /api/admin/waitlist/export.csv`. On `/api/auth/register`, looks up waitlist entry by email and auto-grants `founderBadge=true`/`founderBadgeAt=now` then `convertWaitlistEntryToUser`. `users.founderBadge`/`founderBadgeAt` columns; `FounderBadge` component at `client/src/components/founder-badge.tsx` shown in header user dropdown. Admin "Waitlist" tab in `/admin` (Sparkles icon) with stats cards, entries table with search, top referrers, and CSV export. Admins + logged-in users always bypass the gate.
